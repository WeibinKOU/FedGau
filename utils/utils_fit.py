...
def fit_one_epoch(model, train_util, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir):
    ...
    val_loss = 0
    #print('Start Train')
    with tqdm(total=epoch_step,desc=f'[Train] [Epoch: {epoch}/{Epoch}]',postfix=dict,mininterval=0.3) as pbar:
    ...
    #with tqdm(total=epoch_step_val, desc=f'[Valid] [Epoch: {epoch}/{Epoch}]',postfix=dict,mininterval=0.3) as pbar:
...
#
    ...
    ap_dict = eval_callback.on_epoch_end(epoch + 1)
    ...
    return total_loss / epoch_step, val_loss / epoch_step_val, ap_dict
